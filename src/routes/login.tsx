...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Entrar no Rolei" }] }),
  component: LoginPage,
});
...
async function postLogin(navigate: ReturnType<typeof useNavigate>) {
...
function LoginPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      if (active && data.user) postLogin(navigate);
    });
    const { data: sub } = supabase.auth.onAuthStateChange((_e, session) => {
      if (session?.user) postLogin(navigate);
    });
    return () => {
      active = false;
  ...
  return (
    <div className="app-shell shadow-card min-h-screen bg-background">
      <header className="bg-gradient-hero px-5 pb-10 pt-6 text-white">
        <Link to="/" className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/15 backdrop-blur">
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <div className="mt-6 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest opacity-90">
          <Sparkles className="h-3.5 w-3.5" /> Rolei
        </div>
        ...
        <p className="mt-1.5 text-sm opacity-85">
          {mode === "signin"
            ? "Entre para ver rolês perfeitos para hoje."
            : "Personalize sua experiência e salve seus favoritos."}
        </p>
      </header>
      ...
      <main className="-mt-6 px-5">
        <div className="rounded-3xl bg-card p-5 shadow-card">
          ...
          <form onSubmit={submit} className="mt-5 space-y-3">
            ...
            <Field icon={Mail} type="email" placeholder="email@exemplo.com" value={email} onChange={setEmail} />
            <Field icon={Lock} type="password" placeholder="Senha (mín. 8)" value={password} onChange={setPassword} />
            {mode === "signin" && (
          ...
          <div className="space-y-2.5">
            <OAuthBtn loading={oauthLoading === "google"} onClick={() => oauth("google")} label="Continuar com Google" variant="light" icon={<GoogleIcon />} />
            <OAuthBtn loading={oauthLoading === "apple"} onClick={() => oauth("apple")} label="Continuar com Apple" variant="dark" icon={<AppleIcon />} />
          </div>
        </div>

        <p className="mt-5 px-2 text-center text-[11px] text-muted-foreground">
          Tem um estabelecimento?{" "}
          <Link to="/auth" className="font-bold text-primary">Entrar como parceiro</Link>
        </p>
      </main>
    </div>
  );
...
function OAuthBtn({
  ...
  loading: boolean; onClick: () => void; label: string; variant: "light" | "dark"; icon: React.ReactNode;
}) {
